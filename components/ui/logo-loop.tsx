...
export const LogoLoop = React.memo<LogoLoopProps>(
  ({
    ...
    useResizeObserver(updateDimensions, [containerRef, seqRef], [itemsList, gap, logoHeight, isVertical]);

    useImageLoader(seqRef, updateDimensions, [itemsList, gap, logoHeight, isVertical]);
        ...
        const content = isNodeItem ? (
          <span
            className={cx(
              ...
              scaleOnHover &&
                'transition-transform duration-300 ease-out group-hover/item:scale-120 will-change-transform'
            )}
    ...
    const logoLists = useMemo(
      () =>
        Array.from({ length: copyCount }, (_, copyIndex) => (
          ...
          >
            {itemsList.map((item, itemIndex) => renderLogoItem(item, `${copyIndex}-${itemIndex}`))}
          </ul>
        )),
      [copyCount, itemsList, renderLogoItem, isVertical]
    );
